...
const CHALLENGE_ICONS: Record<string, React.ComponentType<any>> = {
  'first-steps': Target, 'daily-practice': Trophy, 'meditation-master': Brain,
  'gratitude-champion': Heart, 'breath-warrior': Wind, default: Shield,
};
...
export const QuestChallengeContainer: React.FC = () => {
  ...
  const loadQuests = useCallback(() => {
    ...
    if (saved) {
      ...
      } catch (e) { console.error('Failed to load quest progress:', e); }
    }
    setQuests(questData);
  }, []);
    ...
    <>
      ...
      <AnimatePresence>
        {isQuestModalOpen && selectedQuest && (
          <div className="fixed inset-0 z-50">{renderQuestModal()}</div>
        )}
      </AnimatePresence>
